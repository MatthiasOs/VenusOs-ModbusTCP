...
class ModbusOperationTest {
    ...
    private static final DBusUnit V_DC = new DBusUnit("V DC");
    private static final DBusUnit CELSIUS = new DBusUnit("Degrees celsius");
    private static final DBusUnit W = new DBusUnit("W");
    private static final DBusUnit KWH = new DBusUnit("kWh");
    private static final DBusUnit A_DC = new DBusUnit("A DC");
    private static final DBusUnit VA = new DBusUnit("VA");
    ...
    private static final ModbusOperation BAT_BATTERY_VOLTAGE = new ModbusOperation(Category.BATTERY, "Battery voltage", 259, Type.UINT16, 100D, "", "/Dc/0/Voltage",
            AccessMode.READ_ONLY, V_DC, "");
    private static final ModbusOperation GRI_GRID_L1_POWER = new ModbusOperation(Category.GRID, "Grid L1 Power", 2600, Type.INT16, 1D, "", "/Ac/L1/Power", AccessMode.READ_ONLY,
            W, "");
    ...
    private static final ModbusOperation SOL_PV_CURRENT = new ModbusOperation(Category.SOLARCHARGER, "PV current", 777, Type.INT16, 10D, "", "/Pv/I", AccessMode.READ_ONLY, A_DC,
            "");
    private static final ModbusOperation SOL_YIELD_TODAY = new ModbusOperation(Category.SOLARCHARGER, "Yield today", 784, Type.UINT16, 10D, "", "/History/Daily/0/Yield",
            AccessMode.READ_ONLY, KWH, "");
    private static final ModbusOperation SYS_BATTERY_CURRENT_SYSTEM = new ModbusOperation(Category.SYSTEM, "Battery Current (System)", 841, Type.INT16, 10D, "",
            "/Dc/Battery/Current", AccessMode.READ_ONLY, V_DC, "");
    private static final ModbusOperation SYS_AC_CONSUMPTION_L1 = new ModbusOperation(Category.SYSTEM, "AC Consumption L1", 817, Type.UINT16, 1D, "", "/Ac/Consumption/L1/Power",
            AccessMode.READ_ONLY, W, "");
    private static final ModbusOperation VEB_INPUT_POWER_1 = new ModbusOperation(Category.VEBUS, "Input power 1", 12, Type.INT16, 0.1D, "", "/Ac/ActiveIn/L1/P",
            AccessMode.READ_ONLY, VA, "");
    private static final ModbusOperation VEB_BATTERY_VOLTAGE = new ModbusOperation(Category.VEBUS, "Battery voltage", 26, Type.UINT16, 100D, "", "/Dc/0/Voltage",
            AccessMode.READ_ONLY, V_DC, "");
    private static final ModbusOperation TEM_TEMPERATURE = new ModbusOperation(Category.TEMPERATURE, "Temperature", 3304, Type.INT16, 100D, "", "/Temperature",
            AccessMode.READ_ONLY, CELSIUS, "");

    @ParameterizedTest
    @MethodSource({"battery", "grid", "solar", "system", "temp", "veBus"})
    void checkValue(ModbusOperation operation, Integer registerValue, Double value) {
    ...
    private static Stream<Arguments> battery() {
        return of(
    ...
    private static Stream<Arguments> grid() {
        return of(
    ...
    private static Stream<Arguments> solar() {
        return of(
    ...
    private static Stream<Arguments> system() {
        return of(
    ...
    private static Stream<Arguments> veBus() {
        return of(
